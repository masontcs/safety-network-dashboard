            ...
            {[...detail].sort((a, b) => b.amount - a.amount).map((row) => (
              <tr key={row.employeeId} style={{ borderTop: '1px solid #2a2a2a' }}>
                <td className="table-body" style={{ padding: '7px 8px' }}>
                  <a
                    href={`/manager/employees/${row.employeeId}`}
            ...
            <tbody>
              {(isMobile ? topConsumers.slice(0, 5) : topConsumers).map((r) => (
                <tr key={r.employeeId} style={{ borderTop: '1px solid #2a2a2a' }}>
                  <td className="table-body" style={{ padding: '7px 8px' }}>
                    <a
                      href={`/manager/employees/${r.employeeId}`}
                      style={{ color: '#ff6b00', textDecoration: 'none' }}
                      onMouseOver={(e) => (e.currentTarget.style.textDecoration = 'underline')}
                      onMouseOut={(e) => (e.currentTarget.style.textDecoration = 'none')}
                    >
...
export default function ManagerDashboard({ branchId, entityId }: Props) {
  ...
  // On mount: load fiscal months, quarters, and years
  useEffect(() => {
    Promise.all([
  ...
  // Fetch allocation for this branch when toggle is ON
  useEffect(() => {
    ...
    fetch(`/api/allocation/summary?periodDate=${endDate}`)
      ...
      .then((json) => {
        ...
        const found = json.data.allocations.find((a: { branchId: string; totalAllocation: number }) => a.branchId === branchId)
        setBranchAllocAlloc(found?.totalAllocation ?? 0)
      })
      .catch(() => {})
  }, [allocationOn, endDate, branchId])
  ...
  })), [saturdays, revByWeek, weeklyPayroll, fuelByWeek, viewMode])
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div className="card">
        <div style={{ fontSize: 14, fontWeight: 500, color: '#ffffff', marginBottom: 12 }}>
          ...
          {viewMode !== 'year' && <span style={{ fontSize: 11, color: '#555555', fontWeight: 400 }}> · click a bar to see detail</span>}
        </div>
            ...
            <div style={viewMode === 'year' && barData.length > 20 ? { minWidth: barData.length * 20 } : {}}>
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={barData} barCategoryGap="25%" margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                  ...
                  <Bar
                    dataKey="revenue" name="Revenue" fill="#ff6b00" radius={[3, 3, 0, 0]}
                    cursor={viewMode !== 'year' ? 'pointer' : undefined}
                    onClick={viewMode !== 'year' ? (entry: { periodDate: string }) => setSelectedWeek((p) => p === entry.periodDate ? null : entry.periodDate) : undefined}
                  >
                    {barData.map((e) => <Cell key={e.periodDate} fill={viewMode !== 'year' && selectedWeek === e.periodDate ? '#ffaa44' : '#ff6b00'} />)}
                  </Bar>
