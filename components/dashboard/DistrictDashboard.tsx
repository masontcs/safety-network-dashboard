...
interface Props { branches: BranchInfo[]; initialBranch: string }
...
function FuelAnalyticsSection({ fuelByWeekClient, topConsumers, vendorTotals, fuelWeekApiData, saturdays, loading, isMobile }: {
...
function PayrollAnalyticsSection({ hoursWeekly, weeklyPayroll, saturdays, overtimeSummary, directLaborDetail, loading, isMobile }: {
            ...
            {loading ? <Skeleton height={120} /> : (
              <ResponsiveContainer width="100%" height={120}>
                <BarChart data={hoursBarData} barCategoryGap="25%" margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                  ...
                  <Bar dataKey="standard" name="Standard" stackId="a" fill="#ff6b00" />
                  <Bar dataKey="overtime" name="Overtime" stackId="a" fill="#ffaa00" />
...
export default function DistrictDashboard({ branches, initialBranch }: Props) {
  const validInitialBranch =
    initialBranch === 'all' || branches.some((b) => b.id === initialBranch) ? initialBranch : 'all'
  ...
  const [weeklyPayrollByBranch, setWeeklyPayrollByBranch] = useState<Record<string, Record<string, WeekPayroll>>>({})
  ...
  useEffect(() => {
    ...
    const analyticsParams = new URLSearchParams({ startDate, endDate })
    if (!isAggregate) analyticsParams.set('branchId', selectedBranchId)
    ...
    for (const b of targetBranches) {
      for (const sat of weeks) {
        ...
        payrollCalls.push(
          fetch(`/api/payroll/summary?${new URLSearchParams({ branchId: b.id, periodDate: sat, entityId: b.entityId })}`).then((r) => r.json())
        )
      }
    }
    ...
    Promise.all([
      ...
      fetch(`/api/fuel/by-week?${analyticsParams}`).then((r) => r.json()),
      fetch(`/api/fuel/top-consumers?${analyticsParams}`).then((r) => r.json()),
      fetch(`/api/payroll/hours-by-week?${analyticsParams}`).then((r) => r.json()),
      fetch(`/api/payroll/overtime-summary?${analyticsParams}`).then((r) => r.json()),
      fetch(`/api/payroll/direct-labor-detail?${analyticsParams}`).then((r) => r.json()),
      ...payrollCalls,
    ])
  ...
  const fuelByBranch = useMemo(() => {
    const m: Record<string, number> = {}
    for (const t of fuelTxns) if (t.branch_id) m[t.branch_id] = (m[t.branch_id] ?? 0) + t.total_with_tax
    return m
  }, [fuelTxns])

  const payrollByBranch = useMemo(() => {
    const m: Record<string, { direct: number; admin: number; tax: number }> = {}
    for (const [bId, bWeeks] of Object.entries(weeklyPayrollByBranch)) {
      m[bId] = {
        direct: Object.values(bWeeks).reduce((s, w) => s + w.directTotal, 0),
        admin: Object.values(bWeeks).reduce((s, w) => s + w.adminTotal, 0),
        tax: Object.values(bWeeks).reduce((s, w) => s + w.taxTotal, 0),
      }
    }
    return m
  ...
  const fuelByWeek = useMemo(() => {
    const m: Record<string, number> = {}
    for (const t of fuelTxns) { const sat = snapToSat(t.transaction_date); m[sat] = (m[sat] ?? 0) + t.total_with_tax }
    return m
  }, [fuelTxns])
  ...
  // Aggregated weekly payroll across all branches (for payroll cost line chart in aggregate mode)
  const aggregatedWeeklyPayroll = useMemo(() => {
    const m: Record<string, WeekPayroll> = {}
    for (const bWeeks of Object.values(weeklyPayrollByBranch)) {
      for (const [sat, wp] of Object.entries(bWeeks)) {
        if (!m[sat]) m[sat] = { directTotal: 0, adminTotal: 0, taxTotal: 0, detail: [] }
        m[sat].directTotal += wp.directTotal
        m[sat].adminTotal += wp.adminTotal
        m[sat].taxTotal += wp.taxTotal
      }
    }
    return m
  }, [weeklyPayrollByBranch])
  ...
  const trendData = useMemo(() => saturdays.map((sat) => {
    ...
    return {
      label: fmtShort(sat),
      revenue: revByWeek[sat]?.total ?? 0,
      payroll: (wp?.directTotal ?? 0) + (wp?.adminTotal ?? 0) + (wp?.taxTotal ?? 0),
      fuel: fuelByWeek[sat] ?? 0,
    }
  }), [saturdays, revByWeek, fuelByWeek, isAggregate, aggregatedWeeklyPayroll, singleBranchWeeklyPayroll])

  // Per-branch trend data for BranchComparisonCard mini charts
  const branchTrendData = useMemo(() => {
    const result: Record<string, Array<{ label: string; revenue: number; payroll: number; fuel: number }>> = {}
    for (const b of branches) {
      const bRevByWeek: Record<string, number> = {}
      for (const t of revTxns) {
        if (t.branch_id === b.id) bRevByWeek[t.period_date] = (bRevByWeek[t.period_date] ?? 0) + t.total_revenue
      }
      const bFuelByWeek: Record<string, number> = {}
      for (const t of fuelTxns) {
        if (t.branch_id === b.id) {
          const sat = snapToSat(t.transaction_date)
          bFuelByWeek[sat] = (bFuelByWeek[sat] ?? 0) + t.total_with_tax
        }
      }
      result[b.id] = saturdays.map((sat) => {
        const bWp = weeklyPayrollByBranch[b.id]?.[sat]
        return {
          label: fmtShort(sat),
          revenue: bRevByWeek[sat] ?? 0,
          payroll: (bWp?.directTotal ?? 0) + (bWp?.adminTotal ?? 0) + (bWp?.taxTotal ?? 0),
          fuel: bFuelByWeek[sat] ?? 0,
        }
      })
    }
    return result
  }, [branches, revTxns, fuelTxns, weeklyPayrollByBranch, saturdays])

  const selectedWeekData = selectedWeek && !isAggregate ? (singleBranchWeeklyPayroll[selectedWeek] ?? null) : null
  ...
  if (fiscalMonths.length === 0 && !loading) {
    return (
      <div style={{ padding: 32 }}>
        <div style={{ fontSize: 22, fontWeight: 500, color: '#ffffff', marginBottom: 16 }}>{pageTitle}</div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {loading ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            ...
            <div className="card" style={{ padding: '12px 14px', minHeight: 80 }}>
              ...
              <div style={{ fontSize: 20, fontWeight: 500, color: '#ffffff' }}>{noData ? '—' : formatCurrency(totalPayroll)}</div>
              {!noData && totalAdmin > 0 && (
                <div style={{ fontSize: 10, color: '#555555', marginTop: 2 }}>Admin: {formatCurrency(totalAdmin)} lump</div>
              )}
            </div>
            <div className="card" style={{ padding: '12px 14px', minHeight: 80 }}>
        ...
        {isAggregate ? (
          <div className="card" style={{ padding: 0 }}>
            <div style={{ padding: '12px 16px', borderBottom: '1px solid #2a2a2a', fontSize: 13, fontWeight: 500, color: '#ffffff' }}>Branch Performance</div>
            {loading ? (
                  ...
                  <div key={b.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', borderBottom: '1px solid #2a2a2a', minHeight: 44 }}>
                    ...
                    {bNoData ? (
                      ...
                      <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                        ...
                        <span style={{ fontSize: 12, fontWeight: 500, color: gpColor(bGPPct), background: `${gpColor(bGPPct)}1a`, borderRadius: 4, padding: '2px 7px' }}>{bGPPct.toFixed(1)}%</span>
                      </div>
                    )}
                  </div>
                )
        ...
        <PayrollAnalyticsSection
          ...
          weeklyPayroll={isAggregate ? aggregatedWeeklyPayroll : singleBranchWeeklyPayroll}
          saturdays={saturdays}
        ...
        {!isAggregate && <RevBreakdownTable saturdays={saturdays} revByWeek={revByWeek} loading={loading} />}
      </div>
    )
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr 1fr', gap: 12 }}>
        ...
        {loading ? <Skeleton height={140} borderRadius={12} /> : (
          <div className="card" style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ fontSize: 26, fontWeight: 500, color: noData ? '#888888' : '#ffffff', marginTop: 8 }}>
              {noData ? '—' : formatCurrency(totalPayroll)}
            </div>
            {!noData && totalRev > 0 && (
              <div style={{ fontSize: 11, color: '#cc4444', marginTop: 2 }}>
                {formatPercent((totalPayroll / totalRev) * 100)} of revenue
              </div>
            )}
            {!noData && (
              <div style={{ fontSize: 10, color: '#666666', marginTop: 4, display: 'flex', flexDirection: 'column', gap: 1 }}>
                <span>Direct: {formatCurrency(totalDirect)}</span>
                {totalAdmin > 0 && <span>Admin (H+S): {formatCurrency(totalAdmin)} · lump sum</span>}
                {totalTax > 0 && <span>Taxes: {formatCurrency(totalTax)}</span>}
              </div>
            )}
          </div>
        )}
      ...
      {/* ── Aggregate view: branch comparison cards + district totals ── */}
      {isAggregate && (
        <>
          <div style={{ fontSize: 14, fontWeight: 500, color: '#ffffff' }}>Branch Comparison — {periodLabel}</div>
          {loading ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 12 }}>
              {branches.map((_, i) => <Skeleton key={i} height={200} borderRadius={12} />)}
            </div>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 12 }}>
              {[...branches]
          ...
          {/* District Totals table */}
          {!loading && !noData && (
            <div className="card">
              <div style={{ fontSize: 14, fontWeight: 500, color: '#ffffff', marginBottom: 12 }}>District Totals</div>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    {[
                      ...
                      <th key={h.label} className="table-header" style={{ textAlign: h.align as 'left' | 'right', padding: '0 10px 8px', fontWeight: 400, whiteSpace: 'nowrap' }}>{h.label}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    .map((b) => {
                      ...
                      return (
                        <tr key={b.id} style={{ borderTop: '1px solid #2a2a2a' }}>
                          <td className="table-body branch-name" style={{ padding: '9px 10px', fontWeight: 500, whiteSpace: 'nowrap' }}>{b.name}</td>
                          <td className="table-body" style={{ padding: '9px 10px', textAlign: 'right', color: '#ffffff', fontWeight: 500 }}>{formatCurrency(rev)}</td>
                          <td className="table-body" style={{ padding: '9px 10px', textAlign: 'right' }}>{formatCurrency(direct)}</td>
                          <td className="table-body" style={{ padding: '9px 10px', textAlign: 'right', color: '#888888' }}>{formatCurrency(admin)}</td>
                          <td className="table-body" style={{ padding: '9px 10px', textAlign: 'right' }}>{formatCurrency(fuel)}</td>
                          <td className="table-body" style={{ padding: '9px 10px', textAlign: 'right', color: gp >= 0 ? '#cccccc' : '#cc4444' }}>{formatCurrency(gp)}</td>
                          <td className="table-body" style={{ padding: '9px 10px', textAlign: 'right' }}>
                            {gpPct !== null ? <span style={{ color: gpColor(gpPct) }}>{formatPercent(gpPct)}</span> : <span style={{ color: '#555555' }}>—</span>}
                          </td>
                        </tr>
                      )
                    })}
                  <tr style={{ borderTop: '1px solid #333333' }}>
                    ...
                    <td style={{ padding: '9px 10px', textAlign: 'right', fontSize: 12, fontWeight: 500, color: '#ff6b00' }}>{formatCurrency(totalRev)}</td>
                    <td style={{ padding: '9px 10px', textAlign: 'right', fontSize: 12, color: '#cccccc' }}>{formatCurrency(totalDirect)}</td>
                    <td style={{ padding: '9px 10px', textAlign: 'right', fontSize: 12, color: '#888888' }}>{formatCurrency(totalAdmin)}</td>
                    <td style={{ padding: '9px 10px', textAlign: 'right', fontSize: 12, color: '#cccccc' }}>{formatCurrency(totalFuel)}</td>
                    <td style={{ padding: '9px 10px', textAlign: 'right', fontSize: 12, fontWeight: 500, color: grossProfit >= 0 ? '#cccccc' : '#cc4444' }}>{formatCurrency(grossProfit)}</td>
                    <td style={{ padding: '9px 10px', textAlign: 'right', fontSize: 12, color: grossProfitPct !== null && grossProfitPct >= 0 ? '#ff6b00' : '#cc4444' }}>
      ...
      {/* Single branch: revenue breakdown table */}
      {!isAggregate && <RevBreakdownTable saturdays={saturdays} revByWeek={revByWeek} loading={loading} />}

      {/* Fuel Analytics (both views) */}
      <FuelAnalyticsSection
        fuelByWeekClient={fuelByWeek}
        topConsumers={topConsumers}
        vendorTotals={vendorTotals}
      ...
      {/* Payroll Analytics + Direct Labor Detail (both views) */}
      <PayrollAnalyticsSection
        hoursWeekly={hoursWeekly}
        weeklyPayroll={isAggregate ? aggregatedWeeklyPayroll : singleBranchWeeklyPayroll}
        saturdays={saturdays}
