...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    if (!start_date || !isSaturday(start_date)) {
      return NextResponse.json({ success: false, error: 'start_date must be a Saturday (YYYY-MM-DD)', code: 'VALIDATION_ERROR' }, { status: 400 })
    }
